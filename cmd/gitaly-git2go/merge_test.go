...
func TestMerge_squash(t *testing.T) {
	...
	baseFile := localrepo.TreeEntry{Path: "file.txt", Content: "b\nc", Mode: "100644"}
	ourFile := localrepo.TreeEntry{Path: "file.txt", Content: "a\nb\nc", Mode: "100644"}
	theirFile1 := localrepo.TreeEntry{Path: "file.txt", Content: "b\nc\nd", Mode: "100644"}
	theirFile2 := localrepo.TreeEntry{Path: "file.txt", Content: "b\nc\nd\ne", Mode: "100644"}
...
func TestMerge_recursive(t *testing.T) {
	...
	for i := 1; i < git2go.MergeRecursionLimit; i++ {
		ours[i] = localrepo.WriteTestCommit(t, localRepo, localrepo.WithParents(ours[i-1], theirs[i-1]), localrepo.WithTreeEntries(
			localrepo.TreeEntry{Path: "base", Content: "base\n", Mode: "100644"},
			localrepo.TreeEntry{Path: "ours", Content: fmt.Sprintf("ours-%d\n", i), Mode: "100644"},
			localrepo.TreeEntry{Path: "theirs", Content: fmt.Sprintf("theirs-%d\n", i-1), Mode: "100644"},
		))
