...
func TestSubmodule(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			command: git2go.SubmoduleCommand{
				AuthorName: string(git.TestUser.Name),
				AuthorMail: string(git.TestUser.Email),
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, commit.Author.Email, git.TestUser.Email)
			require.Equal(t, commit.Committer.Email, git.TestUser.Email)
