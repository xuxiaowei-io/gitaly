...
func TestListConflictFilesHugeDiff(t *testing.T) {
	...
	ourCommitID := localrepo.WriteTestCommit(t, repo, localrepo.WithTreeEntries(
		localrepo.TreeEntry{Path: "a", Mode: "100644", Content: strings.Repeat("a\n", 128*1024)},
		localrepo.TreeEntry{Path: "b", Mode: "100644", Content: strings.Repeat("b\n", 128*1024)},
	))

	theirCommitID := localrepo.WriteTestCommit(t, repo, localrepo.WithTreeEntries(
		localrepo.TreeEntry{Path: "a", Mode: "100644", Content: strings.Repeat("x\n", 128*1024)},
		localrepo.TreeEntry{Path: "b", Mode: "100644", Content: strings.Repeat("y\n", 128*1024)},
	))
