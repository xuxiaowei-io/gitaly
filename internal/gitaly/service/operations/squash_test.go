...
func TestUserSquash_validation(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expErr: status.Error(codes.InvalidArgument, "empty StartSha"),
		},
		{
			desc: "empty EndSha",
			request: &gitalypb.UserSquashRequest{
			...
			expErr: status.Error(codes.InvalidArgument, "empty EndSha"),
		},
		{
			desc: "empty Author",
			request: &gitalypb.UserSquashRequest{
			...
			expErr: status.Error(codes.InvalidArgument, "empty Author"),
		},
		{
