...
func TestApplyGitattributesFailure(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			repo:     repo,
			revision: []byte(""),
			expErr:   status.Error(codes.InvalidArgument, "revision: empty revision"),
		},
		{
			desc:     "unknown revision",
			repo:     repo,
			revision: []byte("not-existing-ref"),
			expErr:   status.Error(codes.InvalidArgument, "revision does not exist"),
		},
		{
			desc:     "invalid revision",
			repo:     repo,
			revision: []byte("--output=/meow"),
			expErr:   status.Error(codes.InvalidArgument, "revision: revision can't start with '-'"),
		},
	}
