...
func TestGetBlob_successful(t *testing.T) {
	...
	smallBlobID := localrepo.WriteTestBlob(t, repo, "", string(smallBlobContents))

	largeBlobContents := bytes.Repeat([]byte{1}, 1024*1024)
	largeBlobID := localrepo.WriteTestBlob(t, repo, "", string(largeBlobContents))
...
func TestGetBlob_invalidRequest(t *testing.T) {
	...
	repo, _ := gittest.CreateRepository(t, ctx, cfg)
	oid := localrepo.WriteTestBlob(t, localrepo.NewTestRepo(t, cfg, repo), "", "something").String()
