...
func RequireTree(tb testing.TB, cfg config.Cfg, repoPath, treeish string, expectedEntries []TreeEntry) {
	...
	for i, entry := range expectedEntries {
		...
		expectedEntries[i].OID = ObjectID(hex.EncodeToString(hasher.Sum(nil)))
	}
...
func WriteTree(tb testing.TB, cfg config.Cfg, repoPath string, entries []TreeEntry) ObjectID {
	tb.Helper()
