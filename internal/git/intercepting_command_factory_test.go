...
func TestInterceptingCommandFactory(t *testing.T) {
	...
	factory := NewInterceptingCommandFactory(t, ctx, cfg, func(execEnv ExecutionEnvironment) string {
		return fmt.Sprintf(
			`#!/usr/bin/env bash
	...
	t.Run("New", func(t *testing.T) {
		...
		cmd, err := factory.New(ctx, repoProto, Command{
			Name: "rev-parse",
	...
	t.Run("NewWithoutRepo", func(t *testing.T) {
		var stdout bytes.Buffer
		cmd, err := factory.NewWithoutRepo(ctx, Command{
			Name: "rev-parse",
			Args: []string{"something"},
			Flags: []Option{
				ValueFlag{Name: "-C", Value: repoPath},
			},
		}, WithStdout(&stdout))
		require.NoError(t, err)
		require.NoError(t, cmd.Wait())
...
func TestInterceptingCommandFactory_GitVersion(t *testing.T) {
	...
	generateVersionScript := func(execEnv ExecutionEnvironment) string {
	...
	realFactory, cleanup, err := NewExecCommandFactory(cfg)
	require.NoError(t, err)
	defer cleanup()
