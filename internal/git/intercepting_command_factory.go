...
var _ CommandFactory = &InterceptingCommandFactory{}

// InterceptingCommandFactory is a CommandFactory which intercepts all executions of Git
// commands with a custom script.
type InterceptingCommandFactory struct {
	realCommandFactory         CommandFactory
	interceptingCommandFactory CommandFactory
	interceptVersion           bool
}

type interceptingCommandFactoryConfig struct {
	opts             []ExecCommandFactoryOption
	interceptVersion bool
}
...
// ExecCommandFactory.
func WithRealCommandFactoryOptions(opts ...ExecCommandFactoryOption) InterceptingCommandFactoryOption {
...
func NewInterceptingCommandFactory(
	...
	ctx context.Context,
	cfg config.Cfg,
	generateScript func(ExecutionEnvironment) string,
	...
	return &InterceptingCommandFactory{
		...
		interceptingCommandFactory: NewCommandFactory(tb, cfg, WithGitBinaryPath(scriptPath)),
...
func (f *InterceptingCommandFactory) New(ctx context.Context, repo repository.GitRepo, sc Command, opts ...CmdOpt) (*command.Command, error) {
	return f.interceptingCommandFactory.New(ctx, repo, sc, append(
		opts, WithEnv(f.realCommandFactory.GetExecutionEnvironment(ctx).EnvironmentVariables...),
	)...)
}

// NewWithoutRepo creates a new Git command using the intercepting script.
func (f *InterceptingCommandFactory) NewWithoutRepo(ctx context.Context, sc Command, opts ...CmdOpt) (*command.Command, error) {
...
func (f *InterceptingCommandFactory) GetExecutionEnvironment(ctx context.Context) ExecutionEnvironment {
...
func (f *InterceptingCommandFactory) GitVersion(ctx context.Context) (Version, error) {
...
// command factory.
func (f *InterceptingCommandFactory) SidecarGitConfiguration(ctx context.Context) ([]ConfigPair, error) {
