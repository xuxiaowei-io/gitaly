...
func createCommand(tb testing.TB, cfg config.Cfg, execCfg ExecConfig, args ...string) *exec.Cmd {
	...
	gitConfig = append(gitConfig,
		ConfigPair{Key: "init.defaultBranch", Value: "master"},
		ConfigPair{Key: "init.templateDir", Value: ""},
		ConfigPair{Key: "user.name", Value: "Your Name"},
		ConfigPair{Key: "user.email", Value: "you@example.com"},
	)
	...
	cmd.Env = append(cmd.Env, ConfigPairsToGitEnvironment(gitConfig)...)
