...
func ObjectHashIsSHA256() bool {
	return DefaultObjectHash.EmptyTreeOID == ObjectHashSHA256.EmptyTreeOID
}

// RequireObjectExists asserts that the given repository does contain an object with the specified
// object ID.
func RequireObjectExists(tb testing.TB, cfg config.Cfg, repoPath string, objectID ObjectID) {
...
// specified object ID.
func RequireObjectNotExists(tb testing.TB, cfg config.Cfg, repoPath string, objectID ObjectID) {
	requireObjectExists(tb, cfg, repoPath, objectID, false)
}

func requireObjectExists(tb testing.TB, cfg config.Cfg, repoPath string, objectID ObjectID, exists bool) {
...
func WriteBlob(tb testing.TB, cfg config.Cfg, testRepoPath string, contents []byte) ObjectID {
