...
func TestWriteCommit(t *testing.T) {
	...
	commitA := WriteTestCommit(t, cfg, repoPath, WithTree(treeA))
	commitB := WriteTestCommit(t, cfg, repoPath, WithTree(treeB))
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			oid := WriteTestCommit(t, cfg, repoPath, tc.opts...)
			...
			if tc.expectedRevUpdate != "" {
				...
				require.Equal(t, oid, ObjectID(text.ChompBytes(updatedOID)))
			}
		})
	}
	...
	t.Run("empty tree gets written", func(t *testing.T) {
		...
		// corruption.
		WriteTestCommit(t, cfg, repoPath, WithBranch("main"))
