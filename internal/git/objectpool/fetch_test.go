...
func TestFetchFromOrigin_dangling(t *testing.T) {
	...
	unreachableBlob := pool.Repo.MustWriteBlob(t, "unreachable")
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			setup: func(t *testing.T) *ObjectPool {
				...
				poolPath := gittest.RepositoryPath(t, pool)
				gittest.WriteCommit(t, cfg, poolPath, gittest.WithBranch("main"))
				return pool
			},
			expectedFields: logrus.Fields{
				"references.dangling": referencedObjectTypes{},
		...
		},
		{
			...
			setup: func(t *testing.T) *ObjectPool {
				cfg, pool, _ := setupObjectPool(t, ctx)
				poolPath := gittest.RepositoryPath(t, pool)
				gittest.WriteCommit(t, cfg, poolPath, gittest.WithReference("refs/dangling/commit"))
				return pool
			},
			expectedFields: logrus.Fields{
				"references.dangling": referencedObjectTypes{
