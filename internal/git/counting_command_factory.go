...
var _ CommandFactory = &CountingCommandFactory{}
...
func NewCountingCommandFactory(tb testing.TB, cfg config.Cfg, opts ...ExecCommandFactoryOption) *CountingCommandFactory {
...
func (f *CountingCommandFactory) New(ctx context.Context, repo repository.GitRepo, sc Command, opts ...CmdOpt) (*command.Command, error) {
	f.m.Lock()
	defer f.m.Unlock()
	f.counts[sc.Name]++
...
func (f *CountingCommandFactory) NewWithoutRepo(ctx context.Context, sc Command, opts ...CmdOpt) (*command.Command, error) {
	f.m.Lock()
	defer f.m.Unlock()
	f.counts[sc.Name]++
