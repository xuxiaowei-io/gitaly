...
func (s HeuristicalOptimizationStrategy) ShouldRepackObjects(ctx context.Context) (bool, RepackObjectsConfig) {
	...
	if featureflag.GeometricRepacking.IsEnabled(ctx) {
		...
		// expire unreachable objects once per day.
		//
		// One exception is object pools: we don't ever delete objects there, which also
		// means that it is unnecessary to ever perform a full repack for them.
		timeSinceLastFullRepack := time.Since(s.info.Packfiles.LastFullRepack)
		if !s.info.IsObjectPool && nonCruftPackfilesCount > 1 && timeSinceLastFullRepack > 24*time.Hour {
		...
		if s.info.IsObjectPool && nonCruftPackfilesCount > 1 && timeSinceLastFullRepack > 7*24*time.Hour {
